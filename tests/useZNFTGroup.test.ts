...
import { useZNFTGroup } from '../src';

describe('useZNFTGroup', () => {
  beforeEach(() => {
    fetchMock.reset();
    cache.clear();
  });
  ...
  it('loads an nft currently in an auction', async () => {
    ...
    const { waitFor, result } = renderHook(() => useZNFTGroup(['2974']));
  ...
  it('correctly loads auction information from uniswap', async () => {
    ...
    const { waitFor, result } = renderHook(() =>
      useZNFTGroup(['2974'], 'id', { loadCurrencyInfo: true })
    );
  ...
  it('shows an error when an NFT cannot be loaded', async () => {
    ...
    const { waitFor, result } = renderHook(() => useZNFTGroup(['2972']));
  ...
  it('caches multiple NFTs being loaded', async () => {
    ...
    const { waitFor, result } = renderHook(() => useZNFTGroup(['1', '2']));
